...
(function () {
    ...
    function installRowsReorder() {
        ...
        CR.call = function (screen, params, calls) {
            ...
            // 4) Чередование. Ниже boundary сейчас лежат только ряды
            //    КУБ/TMDB (Последнее добавление, Огонь, Популярное...).
            //    Вставляем стриминги между ними так, чтобы:
            //      - первым после "Вы смотрели" шёл ряд КУБ/TMDB (не стриминг);
            //      - стриминги шли ПО ОДНОМУ, между ними минимум один КУБ-ряд;
            //      - КУБ-рядов между стримингами может быть 1 или 2 (вразнобой).
            //
            //    Идём указателем cursor по КУБ-рядам от boundary вниз.
            //    Перед вставкой очередного стриминга пропускаем 1-2 КУБ-ряда
            //    (сколько именно — случайно), затем вставляем один стриминг.
            //    Так стриминг никогда не окажется первым и никогда не встанет
            //    вплотную к другому стримингу.
            var cursor = boundary;

            for (var k = 0; k < collections.length; k++) {
                // Сколько родных КУБ/TMDB рядов пропустить перед этим стримингом.
                // Минимум 1 (чтобы не был первым и не липли друг к другу),
                // иногда 2 — тогда подряд идут два кубовых ряда.
                var gap = 1 + Math.floor(Math.random() * 2); // 1 или 2

                // Двигаем cursor вперёд на gap РОДНЫХ рядов. Считаем только
                // элементы из originalSet — вставленные ранее стриминги
                // (их нет в originalSet) не считаем за "разбавитель".
                var passed = 0;
                while (cursor < calls.length && passed < gap) {
                    if (originalSet.has(calls[cursor])) passed++;
                    cursor++;
                }

                // Если родные ряды кончились — стримингов больше, чем есть
                // чем их разбавлять. Остаток дописываем в конец по одному
                // (между ними уже нет КУБ-рядов, но выше вариантов нет).
                calls.splice(cursor, 0, collections[k]);
                cursor++; // встаём за только что вставленным стримингом
            }
        };
    }
