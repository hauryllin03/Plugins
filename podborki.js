...
(function () {
    ...
    // ------------------------------------------------------------------
    //  Перехват ContentRows.call — общий для подборок и "Вы смотрели".
    //  Ставится один раз; continue_movies.js помечает свой callback
    //  свойством __sc_priority = 0 (см. тот файл).
    // ------------------------------------------------------------------
    function installRowsReorder() {
        ...
        // Callback-и штатных "канальных" рядов помечать нечем — они анонимны.
        // Но мы знаем: наши помечены __sc_priority, всё остальное в момент
        // вызова уже лежит в calls. Значит "граница блока Lampa" — это конец
        // непрерывной серии НЕ-наших callback-ов, идущей от начала после
        // now_playing. Проще: находим последний НЕ-наш ряд, который ядро
        // вставило с index<=1, т.е. первые несколько элементов массива.
        //
        // Надёжный признак: наши callback-и уже помечены. Вынимаем их,
        // затем находим позицию, куда вставить: сразу за последним
        // "канальным" рядом. Канальные — те, что ядро вставило в голову
        // массива (индексы 1..K), где K = число не-наших, помеченных как
        // вставленные ядром. Мы их отличаем по флагу __core_channel,
        // который проставляем, оборачивая исходные ContentRows перед call.
        ...
        CR.call = function (screen, params, calls) {
            ...
            var lenBefore = calls.length;

            // Запомним ссылки на "родные" элементы массива до вызова —
            // это TMDB-загрузчики (now_playing, trending, popular...).
            var originalSet = new Set(calls);

            origCall(screen, params, calls);

            // После origCall в calls добавились: канальные ряды Lampa
            // (continue_watch/recomend/timetable) и наши помеченные ряды.
            // 1) Вынимаем наши (по __sc_priority), запоминаем порядок.
            var ours = [];
            ...
            ours.sort(function (a, b) { return a.__sc_priority - b.__sc_priority; });

            // 2) Находим границу блока Lampa: это индекс последнего элемента,
            //    которого НЕ было в originalSet (т.е. добавлен ядром как
            //    канальный ряд), среди ГОЛОВЫ массива — до первого исходного
            //    TMDB-загрузчика, встреченного после хотя бы одного нового.
            //
            //    now_playing стоит на позиции 0 и всегда исходный. Канальные
            //    ряды ядро вставляет с index:1, т.е. начиная с позиции 1.
            //    Идём с позиции 1, пока элементы НЕ из originalSet — это
            //    канальный блок. Первый исходный элемент = его конец.
            ...
            // 3) Вставляем наши ряды одним блоком в найденную позицию.
